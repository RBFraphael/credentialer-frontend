...
export default function EditCredential()
{
    ...
    const loadCredential = () => {
        PrivateApi.get(`/credential/${credentialId}?with[]=files&with[]=project`).then((res: AxiosResponse) => {
    ...
    const onSubmit = (e: FormEvent) => {
        ...
        PrivateApi.put(`/credential/${credentialId}`, payload).then((res: AxiosResponse) => {
            Swal.fire({
                icon: "success",
                title: "Sucesso",
                html: `A credencial <b>${credential.title}</b> foi atualizada com sucesso.`
            }).then(() => {
                router.push(`/clients/${clientId}/projects/${projectId}/credentials`);
            });
        }).catch((error: Error | AxiosError) => {
            let message: string = "Ocorreu um erro durante a atualização da credencial";
                ...
                <div className="row">
                    <div className="col-12">
                        <h1>
                            ...
                            </button> Editar Credencial &quot;{credential.title}&quot;
                        </h1>
                        <hr />
                    </div>
                </div>
                                        ...
                                        <div className="col-12">
                                            { submitting ? (
                                                ...
                                                <button type="submit" className="btn btn-success shadow-sm">
                                                    <FontAwesomeIcon icon={faSave} fixedWidth /> Salvar Credencial
                                                </button>
                                            )}
                                        </div>
                                    </div>
